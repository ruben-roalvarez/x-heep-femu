...
typedef struct {
    ...
    XBuffer_adc_uio_map maps[ MAX_UIO_MAPS ];
} XBuffer_adc_uio_info;

/***************** Variable Definitions **************************************/
static XBuffer_adc_uio_info uio_info;
...
static int uio_info_read_name(XBuffer_adc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBuffer_adc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBuffer_adc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBuffer_adc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBuffer_adc_Initialize(XBuffer_adc *InstancePtr, const char* InstanceName) {
	XBuffer_adc_uio_info *InfoPtr = &uio_info;
